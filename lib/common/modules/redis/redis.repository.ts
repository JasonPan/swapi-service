...
export class RedisRepository implements OnModuleDestroy {
  ...
  async multiExec(commands: unknown[][]): Promise<void> {
    this.redisClient.multi(commands).exec();
  }

  async getCardinality(prefix: string, key: string): Promise<number> {
    return this.redisClient.zcard(`${prefix}:${key}`);
  }
